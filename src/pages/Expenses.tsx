...
export const Expenses: React.FC = () => {
  ...
  const loadExpenses = useCallback(async () => {
    try {
      ...
      // Calculate stats dynamically based on actual categories in expenses
      ...
      setStats({
        ...
        ...categoryTotals // Spread all category totals dynamically
      });
    } catch (error) {
  ...
  // Listen for Master Settings updates to refresh categories
  useEffect(() => {
    const handleMasterSettingsUpdate = () => {
      console.log('Master Settings updated, refreshing expense categories...');
      loadExpenseCategories();
    };

    window.addEventListener('masterSettingsUpdated', handleMasterSettingsUpdate);
    window.addEventListener('expenseCategoryUpdated', handleMasterSettingsUpdate);

    return () => {
      window.removeEventListener('masterSettingsUpdated', handleMasterSettingsUpdate);
      window.removeEventListener('expenseCategoryUpdated', handleMasterSettingsUpdate);
    };
  }, []);
            ...
            <div className="space-y-4">
              {loading ? (
                ...
                filteredExpenses.map((expense) => {
                  // Check if this is an auto-generated salary expense
                  const isAutoGenerated = expense.receipt && (
                    expense.description.includes('Staff Salary:') || 
                    expense.description.includes('Salary Adjustment:') || 
                    expense.description.includes('Staff Bonus:')
                  );
                  
                  return (
                    <Card key={expense.id} className={`hover:shadow-md transition-shadow ${isAutoGenerated ? 'border-blue-200 bg-blue-50/30' : ''}`}>
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center space-x-4 text-sm text-muted-foreground">
                                ...
                                {expense.receipt && (
                                  <div className="flex items-center space-x-1">
                                    ...
                                    <span className="text-blue-600 font-medium">{expense.receipt}</span>
                                  </div>
                                )}
                              </div>
                              {isAutoGenerated && (
                                <p className="text-xs text-blue-600 mt-2">
                                  ℹ️ This expense was automatically created from a staff salary receipt
                                </p>
                              )}
                            </div>
                          </div>
